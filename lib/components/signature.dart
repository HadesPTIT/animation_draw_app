        ...
        child: Listener(
          onPointerDown: (PointerDownEvent event) {
            if (!widget.controller.disabled &&
                (activePointerId == null || activePointerId == event.pointer)) {
...
class SignatureController extends ValueNotifier<List<Point>> {
  ...
  void undo() {
    if (_latestActions.isNotEmpty) {
      final lastAction = _latestActions.removeLast();
  ...
  void redo() {
    if (_revertedActions.isNotEmpty) {
      final lastRevertedAction = _revertedActions.removeLast();
  ...
  Future<ui.Image?> toImage({int? width, int? height}) async {
    ...
    final canvas = Canvas(recorder)
      ..translate(
        -(minXValue! - penStrokeWidth),
        -(minYValue! - penStrokeWidth),
      );
    if (exportBackgroundColor != null) {
      final paint = Paint()..color = exportBackgroundColor!;
  ...
  Future<Uint8List?> toPngBytes({int? height, int? width}) async {
    ...
    final image = await toImage(height: height, width: width);
  ...
  Uint8List? _toPngBytesForWeb({int? height, int? width}) {
    ...
    final backgroundColor = exportBackgroundColor ?? Colors.transparent;
    ...
    final translatedPoints = _translatePoints(points)!;

    final canvasWidth = width ?? defaultWidth!;
    final canvasHeight = height ?? defaultHeight!;
    ...
    for (var i = 0; i < translatedPoints.length - 1; i++) {
  ...
  String? toRawSVG({int? width, int? height}) {
    ...
    final latestActionList = List<List<Point>>.from(_latestActions)
        ...
        .reversed
        .toList();
    final strokes = latestActionList
        .asMap()
